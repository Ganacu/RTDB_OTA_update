...
#define OTA_MAX_RETRIES     5                // Retry OTA this many times before giving up
// ──────────────────────────────────────────────────────────────────────────────
...
float  last_acted      = -1.0;
String last_not_found  = "";
...
void handleVersionLogic(float targetVer) {
  ...
  if (payload.indexOf("\"" + searchKey + "\"") != -1) {
    ...
    ota_pending = true;  // Set BEFORE returning to prevent race condition in loop()
...
void executeOTA() {
  ...
  // Disable watchdog before blocking download
  // httpUpdate.update() blocks 20-90s — WDT mid-flash causes corrupted firmware
  ...
  for (int attempt = 1; attempt <= OTA_MAX_RETRIES; attempt++) {
    Serial.printf("[OTA] Attempt %d of %d...\n", attempt, OTA_MAX_RETRIES);

    // Fresh client each attempt — avoids stale connection from previous failure
    ...
    Serial.printf("[OTA] Attempt %d failed (code %d): %s\n",
      attempt,
      httpUpdate.getLastError(),
      httpUpdate.getLastErrorString().c_str());

    if (attempt < OTA_MAX_RETRIES) {
      Serial.println("[OTA] Waiting 5s before next attempt...");
      delay(5000);
    }
  }

  // All attempts exhausted — stay on current version
  Serial.printf("[OTA] All %d attempts failed. Staying on v%.1f.\n",
                OTA_MAX_RETRIES, current_version);
  Serial.println("[OTA] Check the URL in Firestore or your file server and try again.");
  ...
  // Tell RTDB we are still on current_version
  Firebase.RTDB.setFloat(&writeData, "/firmware/active_version", current_version);
  esp_task_wdt_reset();

  // Reset OTA state fully
  ota_pending    = false;
  ota_url        = "";
  ota_version    = 0.0;
  last_acted     = current_version;  // Silence further triggers for this version
  last_not_found = "";

  // Restart web server and stream (were stopped before OTA attempt)
  server.on("/", []() {
